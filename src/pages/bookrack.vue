<template>
	<div class="bookrack">
		...
		<div class="subscription">
			<div class="item f-align" @click="toRead(item)" v-for="(item, index) in bookList"
			     :class="{'show-op':item.showOperation}" :key="index">
				<div class="content f-align f-s-b">
					...
					<divc class="f-align">
						<i class="icon iconfont icon-daipingjia01 pinjia" v-if="item.behavior && item.behavior.grade < 0"
						   @click.stop="pinjia(item, index)"></i>
						<img class="more" :src="iconMore" @click.stop="showOperation(index)"/>
					
					</divc>
				</div>
				<div class="extra flex">
					<div class="operation" :class="{gray: item.isMain}" @click.stop="overhead(item, index)">{{item.isMain ? '取消顶置'
						: '顶置'}}
					</div>
