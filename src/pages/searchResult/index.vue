<template>
	<div class="search-result">
		<div class="search-box van-hairline--bottom f-s-b">
			<v-s class="search-input f-g-1" :value="searchText" @blur="onBlur" @focus="onFocus" @search="onSearch"
			     @cancel="onCancel" placeholder="请输入搜索关键词"/>
			<span class="c-b" @click="onCancel">取消</span>
		</div>
		<div class="container">
			<div class="f-align-l base van-hairline--bottom" :key="index" @click="toDetail(item)" v-if="searchResult.length"
			     v-for="(item, index) in searchResult">
				<img :src="item.mainImg.url"/>
				<div class="f-g-1">
					<p class="title"><span v-html="item.title"></span></p>
					<p class="extra"><span v-html="item.author"></span><span class="dot"></span>{{item.mainTag}}<span
							class="dot"></span>{{item.status ? '连载' : '完结'}}<span class="dot"></span>{{(item.totalCount/
