<template>
	<div class="reading" v-if="chapter">
		...
		<section class="content" :style="{background: status.bg, color: status.color, fontSize: fontConfig.fontSize + 'rpx'}" @longpress="handleLongpress" @tap="hideOperation">
			<ul class="f-align opeartion van-hairline--surround">
				...
				<li class="f-align" @click="next">{{nowCount < total ? '下一章' : '没有咯'}}<i class="iconfont icon-icon_arrow_right"></i></li>
			</ul>
			<h1>{{chapter.title}}</h1>
			<div  class="text">
		...
		>
			<div class="book-info f-align-l" :style="{background: status.listTitleBg}" @click="$router.replace({path: '/pages/bookDetail/index?bookId=' + $route.query.bookId})">
			...
			<LoadSection :height="'calc(100% - 314rpx)'" @toRead="toRead" :showChapterNum="nowCount" noBorder :color="[status.color, status.listBg]" class="load-box" @loading="loading" :sectionList="sectionList"/>
		</van-popup>
		<footer v-show="operationShow" :style="{background: status.operationBg, color: status.color}">
			<div>
				...
				<div class="f-align">
					...
					<van-slider class="f-g-1" use-button-slot :inactive-color="status.barBg" :active-color="status.activeBarBg" :min="1 / total" :value="(nowCount / total.toFixed(2) * 100)" @change="sectionChange">
		...
		<footer v-show="settingShow" class="setting" :style="{background: status.operationBg, color: status.color}">
			<div>
				<div class="f-align fontsize">
					...
					<van-slider class="f-g-1" use-button-slot :inactive-color="status.barBg" :active-color="status.activeBarBg" :value="fontConfig.progressValue" @drag="fontChangeHandler">
						<div class="custom-button" :style="{background: status.activeBarBg}" slot="button">
						</div>
					</van-slider>
...
<script>
  ...
  import Comment from '@/components/Comment.vue'
    ...
    onUnload() {
      ...
      comment () {
        this.$router.push({path: '/pages/comment/index', query: {chapterId: this.chapter._id, chapterName: this.chapter.title, bookId: this.chapter.bookId } })
      },
			catalogue () {
