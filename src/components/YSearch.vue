	...
	<div class="y-search">
		<van-popup :show="show" @close="$emit('transShow', false)" position="right">
			<div class="search-box van-hairline--bottom f-s-b">
				<van-search v-if="show" class="search-input f-g-1" @change="onChange" @blur="onBlur" @focus="onFocus"
				            @search="onSearch" @cancel="onCancel" placeholder="请输入搜索关键词"/>
			...
			<div class="search-result" v-if="!resultShow">
				<div class="history" v-if="searchHistory.length">
					...
					<div class="box van-hairline--surround" @click="hSearch(item)" v-for="(item, index) in searchHistory"
					     :key="index">{{item}}
					</div>
				</div>
			...
			<div v-if="resultShow">
				...
				<ul class="search-helper van-hairline--top-bottom" v-if="preSearch && preSearch.authorBooks">
					<li @click="bookDetail(item)" v-for="(item, index) in preSearch.authorBooks" :key="item._id"
					    class="van-hairline--bottom f-align-l"><i class="iconfont icon-tuijian"></i>{{item.title}}
					</li>
				</ul>
				...
				<ul class="search-helper van-hairline--top-bottom" v-if="preSearch && preSearch.searchBooks">
					<li @click="bookDetail(item)" v-for="(item, index) in preSearch.searchBooks" :key="item._id"
					    class="van-hairline--bottom f-align-l"><i class="iconfont icon-tuijian"></i>{{item.title}}
					</li>
				</ul>
			</div>
