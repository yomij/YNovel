<template>
	<div class="commit f-a-s" v-if="comment && comment.user">
		<img :style="{width: imgSize +'rpx', height: imgSize +'rpx', 'min-width': imgSize +'rpx'}"
		     class="van-hairline--surround" :src="comment.user.avatarUrl"/>
		<div class="c-content van-hairline--bottom f-g-1">
			...
			<div class="f-s-b more-info">
				...
				<div class="f-align" @click="like(comment.isLike)"><i class="iconfont icon-dianzan"
				                                                      :class="{'c-r': comment.isLike}"></i>
					{{comment.likeCount}}
				</div>
			</div>
